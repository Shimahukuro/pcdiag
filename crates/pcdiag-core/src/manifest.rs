...
pub const CURRENT_ARTIFACT_SCHEMA_VERSION: &str = "2.3";
...
mod tests {
    ...
    fn rejects_artifact_schema_version_1() {
        ...
        assert!(errors.errors().iter().any(|error| {
            error.path == "/artifact_schema_version"
                && error
                    .message
                    .contains("supported versions are 2.0 through 2.3")
        }));
    }
    ...
    fn rejects_future_minor_and_malformed_schema_versions_with_details() {
        ...
        future.artifact_schema_version = "2.4".into();
        let error = future.validate().unwrap_err().to_string();
        assert!(error.contains("\"2.4\" is unsupported"));
        assert!(error.contains("2.0 through 2.3"));
