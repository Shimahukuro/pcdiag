...
`2.2`では、任意カテゴリ`windows_security`と同名のコレクター列挙値を追加した。Windows Security Centerの6カテゴリとHVCIの構成・実行状態を保持する。カテゴリがない旧成果物は未収集として扱い、保護状態を補完しない。

`2.3`では`windows_security.details`に33の個別観測項目を追加した。現在の出力スキーマは`2.3`であり、`2.0`～`2.2`も読み込める。旧成果物のdetails欠損を正常・無効と推定しない。詳細は[収集範囲](windows-security-coverage.md)を参照する。
