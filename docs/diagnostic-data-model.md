...
スキーマ2.3で`windows_security.details`を追加した。全33キーは[画面との対応表](windows-security-coverage.md)とコードの`DETAIL_FIELDS`を参照する。各値は文字列または理由付きnull。旧成果物ではdetailsを省略可能。新しいコレクター状態は従来の8項目と追加33項目全体で決定する。
...
重大度は、診断規則の条件が成立した所見について、想定される影響と対応の緊急性を表す。異常である確率や診断の確信度、情報収集の成否は表さない。重大度は高い順に次の4段階とする。

| 値 | 定義 | 選択基準 |
| --- | --- | --- |
| `critical` | 即時対応が必要な重大所見 | データ損失、起動不能、回復不能な障害、またはセキュリティ侵害につながる切迫した危険がある。安全な停止、退避、隔離などを直ちに検討する必要がある。 |
| `error` | 早期対応が必要な異常 | 機能低下、構成異常、反復する障害などが既に発生しており、放置すると運用または信頼性へ大きく影響する。 |
| `warning` | 確認または計画的対応が必要な注意事項 | 現時点では継続利用できるが、余裕の減少、望ましくない状態、将来の障害要因があり、確認や計画的な是正を要する。 |
| `information` | 対応を必須としない参考所見 | 異常や差し迫った危険ではないが、利用者の判断に役立つ状態または変化を伝える。 |

診断規則は、検出件数ではなく、条件が成立した1件の所見が要求する対応に基づいて重大度を固定する。同じ条件に対して環境や収集状態だけを理由に重大度を変更しない。重大度を割り当てる場合は規則仕様に選択理由を記載する。

`severity`を持つことができるのは`status == triggered`の評価だけであり、この場合は`null`を許可しない。`passed`、`not_applicable`、`not_evaluated`、`failed`では必ず`null`とする。

### 総合重大度

`summary.overall_severity`は、`triggered`である評価のうち最も高い重大度とする。順位は`critical > error > warning > information`である。該当する評価がなければ`null`とする。

同一重大度の件数、`not_evaluated`または`failed`の存在、収集成果物の`partial`または`failed`を理由に総合重大度を引き上げない。これらは診断範囲や処理状態として別に集計・表示する。

### 収集状態・評価状態との関係

収集状態はデータを取得できたか、評価状態は規則を適用できたか、重大度は成立した所見へどの程度の対応が必要かを表し、互いに独立した概念である。

- 必要な収集値がない場合は`not_evaluated`とし、推測で重大度を付けない。
- 診断規則自体の処理に失敗した場合は`failed`とし、PCの異常として数えない。
- 収集成果物が`partial`でも、取得済みの値から成立した所見の重大度は変更しない。
- 未評価または失敗がある場合は、総合重大度とは別に診断範囲が限定されていることを利用者へ示す。

### 互換性と判定方式

この4値は現在の診断スキーマで定義する閉じた列挙である。重大度の追加、削除、意味または順位の変更は既存の読み手が解釈できない可能性があるため、互換性を破るスキーマ変更として扱い、対応する成果物仕様および規則セットのバージョンを更新する。

重大度は判定方式を表さない。現在の`pcdiag_builtin`は規則ベースの評価だけを出力する。将来AIによる推定を追加する場合は、重大度とは別の出所・判定方式および確信度を表す項目を仕様化し、規則ベースの評価と区別できるようにする。
...
診断ルールセット名は`pcdiag_builtin`とする。メモリ規則のみの初期バージョンを`0.1.0`、GPU規則を追加したバージョンを`0.2.0`、正常時を含むGPU診断根拠を追加したバージョンを`0.2.1`、GPUデバイスインスタンスIDの重複検出を追加したバージョンを`0.3.0`、接続デバイス診断を追加したバージョンを`0.4.0`、Windows実機結果に基づいて開始状態と有効状態を区別し、接続デバイス診断を問題コードへ限定したバージョンを`0.5.0`、SMART、ボリューム空き容量、イベントログ診断を追加したバージョンを`0.6.0`、イベントログ診断を選別・集約したバージョンを`0.7.0`、異常終了イベントを時刻で相関する現在のバージョンを`0.8.0`とする。診断成果物は収集成果物のマニフェストとファイル完全性を検証した後にだけ生成する。

## AI初期診断ガイド

診断成果物には`diagnosis.json`と同じルールセットに対応する`ai-diagnosis-guide.md`を含める。ガイドは評価状態、重大度、根拠、判定条件、全組み込みルール、全推奨コード、安全上の制約、利用者への回答形式を定義する。

AIは最初にガイドと`diagnosis.json`だけを使用する。追加情報が必要な場合も`collection.json`または`status.json`全体を要求せず、`evidence.path`、`evidence.source_paths`、`reason.paths`、ガイドのパスカタログから必要最小限のJSON Pointerを指定する。`status.json`の取得状態を確認する要求では配列番号を推測せず、関連コレクター名とcollection上の対象パスを指定する。

ガイドが宣言するルールセット名・バージョンまたはartifactスキーマバージョンが入力と一致しない場合、AIはパスや推奨対応を推測せず、互換性を確認できないことを利用者へ伝える。
...
Windows基本情報は初期段階では表示および診断根拠として使用し、単独の診断規則は設けない。Windowsエディション、アーキテクチャ、BIOS・UEFI起動方式、稼働時間の長短はいずれも、それだけでは障害を示さない。将来、Windowsのサポート期限や更新状態を判定する場合は、判定基準となる外部情報のバージョンと取得日時を明示できる別規則として追加する。
...
時計情報には初期段階では単独の診断規則を設けない。Windows Timeサービスは、ワークグループ、ドメイン、オンデマンド起動、第三者製時刻同期ソフトウェアなどの構成によって停止していても異常とは限らない。また、収集対象PC自身の時計だけでは時刻の正確性を判定できない。

将来、ハードウェアRTCまたは信頼できる外部時刻との比較値を収集できるようになった場合は、比較元、取得時刻、許容差を明示した時計差規則を追加する。サービス状態はその際の診断根拠として使用できる。
...
ファームウェア情報には初期段階では単独の診断規則を設けない。Legacy BIOS起動、Secure Boot無効、古い公開日のファームウェアは、対象機器の仕様や利用方針によって正常な構成になり得る。`status`も初期実装では信頼できる一般的な取得元がなく、常に取得不能となるため診断には使用しない。

将来、機種を特定せずに利用できる信頼性の高い状態情報、またはベンダーと機種に対応したファームウェア基準を安全に管理できるようになった場合に診断規則を再検討する。
