...
引数なしの一括実行と、`collect`、`diagnose`、`report`の起動時には、GitHub Releasesに公開された新しいバージョンを最大1回確認する。`--help`、引数解析に失敗した場合、および`--internal-collect`で起動する内部コレクターでは確認しない。

`--no-update-check`は位置に依存しない共通オプションとし、指定された場合は更新確認の外部通信を行わない。同じ起動で複数回指定した場合は引数エラーとする。

新しいバージョンがある場合だけ標準エラー出力へ通知し、標準出力と診断処理は変更しない。確認の失敗は終了コードや後続処理へ影響させない。詳細は[`update-check.md`](update-check.md)を参照する。
