...
- Windows セキュリティ6カテゴリとメモリ整合性（HVCI）の構成・実行状態の収集と表示
- GPUとドライバー情報の収集
...
Windows セキュリティは読み取り専用で収集します。保護状態、未監視、サービス停止、権限不足を区別し、メモリ整合性は構成状態と実行状態を別々に表示します。注意表示は診断概要の総合判定には含めません。収集仕様と通常権限での確認手順は[`docs/windows-security.md`](docs/windows-security.md)を参照してください。
